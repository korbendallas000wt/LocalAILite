...
| Модуль | Версия | Роль |
|--------|--------|------|
| main.py | v1.1.0 | Точка входа: QApplication, валидация путей, запуск MainWindow |
| ui/main_window.py | v1.1.0 | Главное окно: 3 вкладки, меню, OllamaManager, VAEManager, SharedBottomBar |
| ui/shared_bottom_bar.py | v1.1.0 | Общая нижняя панель: промпт, прогресс, таймер, RAM/CPU, индикатор ресурса, радиокнопки |
...
| Модуль | Версия | Роль |
|--------|--------|------|
| ui/tabs/ollama_tab.py | v1.1.0 | Чат: ChatWidget + SettingsPanel + OllamaClient, acquire/release ресурса |
| ui/tabs/diffusers_tab.py | v1.1.0 | Генерация: preview + settings + DiffusersWorker, управление чекпоинтами |
...
| core/ollama_manager.py | v1.1.0 | Управление ollama serve, проверка RAM, CPU affinity, nice-приоритет |
| core/diffusers_worker.py | v1.1.0 | QProcess-обёртка для generate_diffusers.py, проверка RAM, CPU limits, history_dir |
...
| core/resource_manager.py | v1.1.0 | Управление ресурсом (GPU/RAM): acquire/release, 3 арендатора |
| core/resource_monitor.py | v1.1.0 | Мониторинг RAM/CPU, оценка потребления, лимиты, CPU affinity |
| core/vae_manager.py | v1.1.0 | VAE Decoder как отдельный арендатор ресурсов (QProcess) |
...
| Модуль | Версия | Роль |
|--------|--------|------|
| scripts/generate_diffusers.py | v1.1.0 | CLI-генерация SDXL, callback_on_step_end, history_dir, resume |
| scripts/vae_decoder_daemon.py | v1.1.0 | VAE Decoder: watchdog + single_file режим |
...
│   └── vae_manager.py                   # VAE Decoder (QProcess)
│
├── scripts/                             # CLI-скрипты (запускаются в venv)
│   ├── generate_diffusers.py            # Генерация SDXL
│   ├── vae_decoder_daemon.py            # VAE Decoder daemon
...
- **SharedBottomBar**: единая нижняя панель для всех табов (промпт, прогресс, таймер, RAM/CPU, индикатор ресурса)
